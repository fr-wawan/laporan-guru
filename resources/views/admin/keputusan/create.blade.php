...
<div class="sm:ml-64 mt-32">
  <div class="w-11/12 mx-auto bg-white p-10 rounded-lg shadow-lg">
    ...
    <h1 class="font-bold mb-5 text-xl border-b pb-3">Create Surat Keputusan</h1>
    <form action="{{ route('admin.keputusan.store') }}" enctype="multipart/form-data" method="post">

      @csrf
      <x-input name="no_sk" label="No SK Pengangkatan : " placeholder="No SK Pengangkatan..." :value="old('no_sk')" />

      <x-input name="tahun" label="Tahun Pengangkatan : " type="date" :value="old('tahun')" />
      ...
      <div class="lg:flex gap-5">
        <div class="my-3 w-full">
          ...
          <select name="lembaga" id="lembaga"
            ...
            <option value="ketua_yayasan" @if (old('lembaga')=='ketua_yayasan' ) selected @endif>Ketua Yayasan</option>
            <option value="kepala_sekolah" @if (old('lembaga')=='kepala_sekolah' ) selected @endif>Kepala Sekolah
            </option>
          </select>
        ...
        <div class="my-3 w-full">
          ...
          <select name="sumber_gaji" id="sumber_gaji"
            ...
            <option value="yayasan" @if (old('sumber_gaji')=='yayasan' ) selected @endif>Yayasan</option>
            <option value="sekolah" @if (old('sumber_gaji')=='sekolah' ) selected @endif>Sekolah</option>
          </select>
